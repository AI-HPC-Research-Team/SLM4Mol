...
class GCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        #print(f"self_loop_attr : {self_loop_attr}")
        #print(f"self_loop_attr.shape : {self_loop_attr.shape}")
        ...
        #the_size = self._check_input(edge_index, (edge_index.shape[0],edge_index.shape[1]))
        #print(the_size)
        #print(edge_index)
        #print(edge_index.shape)
        
        output = self.propagate(edge_index = edge_index, x = x, edge_attr=edge_embeddings, norm = norm)
        #output = self.propagate(edge_index, (edge_index.shape[0],edge_index.shape[1]))
        return output
...
class GATConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        #print(edge_index)
        #print(edge_index.shape)
        output = self.propagate(edge_index = edge_index, x=x, edge_attr=edge_embeddings)
        return output
...
class MoMuGNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin", **kwargs):
        ...
        #for layer in range(num_layer):
            #self.batch_norms.append(torch.nn.InstanceNorm1d(emb_dim)) 
    ...
    def forward(self, *argv):
        ...
        node_representation_attention_mask = []
        ...
        for graph_idx in range(len(node_counts)):
            ...
            node_mask = torch.ones(node_counts[graph_idx], dtype=torch.bool)
            node_representation_attention_mask.append(node_mask)

        node_representation_padded = torch.nn.utils.rnn.pad_sequence(node_representation_list, batch_first=True)
        node_representation_attention_mask_padded = torch.nn.utils.rnn.pad_sequence(node_representation_attention_mask, batch_first=True, padding_value=False)
        return h_graph,node_representation_padded, node_representation_attention_mask_padded
